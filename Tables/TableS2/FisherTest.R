...
input <- read.csv("3000Variants.tsv", sep ='\t',na.strings = '-')
...
df <- input[complete.cases(input[c(11,16)]),]

#Rows with NA in column 11 or 16 are missing gnomAD variants. Keep for final table
notInGnomAD <- input[!complete.cases(input[c(11,16)]),]
...
write.csv(table,"TableS2.csv",row.names = FALSE,na = "-")
